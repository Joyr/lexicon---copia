...
$('#sync-home').bind('pageshow', function(){
    ...
    if(chevre.syncActivated()){
        ...
        $('#sync-passcode-reminder').html($.store.get(SL_user+'$'+SL_KEYS.SYNC_KEY));
    }    
    else{
        //nope, not set up yet
        notyet.show();
...
$('#sync-start').click(function(){
    ...
    if(passcode && passcode.length > 0){
        $.store.set(SL_user+'$'+SL_KEYS.SYNC_KEY, passcode);
...
$('#sync-download').click(chevre.syncDownload);
$('#sync-stop').click(function(){
    //cancel sync - destroy key
    $.store.remove(SL_user+'$'+SL_KEYS.SYNC_KEY);
});
...
$('.flashcard-image').attr('title', MSG_JS.MSG_full_size);
}
